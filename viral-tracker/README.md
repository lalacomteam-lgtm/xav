...
> 먼저 [python.org](https://www.python.org/downloads/)에서 Python을 설치하세요.
> (윈도우는 설치 화면에서 **"Add python.exe to PATH"** 체크 필수!)

1. 저장소를 내려받아 압축을 풉니다 (GitHub의 초록색 **Code** 버튼 → **Download ZIP**)
2. `viral-tracker` 폴더에서:
   - **윈도우**: `시작하기.bat` 더블클릭
   - **맥**: `시작하기.command` 우클릭 → 열기 (첫 실행만)
3. 처음 한 번은 자동으로 구성요소를 설치하고(몇 분 소요), 이후 브라우저가 자동으로 열립니다.
4. 검은 창을 닫으면 앱이 종료됩니다. 내려받은 영상은 `viral-tracker/downloads/` 폴더에 있습니다.
